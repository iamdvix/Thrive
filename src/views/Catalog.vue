        ...
        <template v-else>
            <section v-if="!isFavoritesMode && saleProducts.length" class="mb-8">
                ...
                <div class="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                    <article v-for="product in saleProducts" :key="`sale-${product.id}`" class="w-[190px] shrink-0 cursor-pointer bg-white p-2 shadow-sm ring-1 ring-rose-100 sm:w-[225px] sm:rounded-[20px]" @click="openProduct(product.id)">
