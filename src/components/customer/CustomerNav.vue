<script setup>
// Navegación del cliente. En computadora vive bajo la búsqueda y en celular se fija al borde inferior.
import { useRouter } from "vue-router";
const props=defineProps({active:{type:String,default:"discover"},favoriteCount:{type:Number,default:0}});
...
<template>
<nav class="customer-nav-desktop items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" aria-label="Navegación del cliente">
    <button v-for="item in items" :key="item.key" type="button" class="flex shrink-0 items-center gap-2 rounded-full border px-4 py-2.5 text-sm font-bold transition" :class="active===item.key?'border-[#00B4D8] bg-[#00B4D8] text-white shadow-sm':'border-[#DCEFF3] bg-white text-[#4F7180] hover:border-[#90E0EF] hover:text-[#0077B6]'" :aria-current="active===item.key?'page':undefined" @click="go(item)">
        ...
        <span>{{ item.label }}</span><span v-if="item.key==='favorites'&&favoriteCount" class="rounded-full px-1.5 py-0.5 text-[9px]" :class="active===item.key?'bg-white/20 text-white':'bg-[#EAF9FC] text-[#0077B6]'">{{ favoriteCount }}</span>
    </button>
</nav>
<Teleport to="body">
    <nav class="customer-nav-mobile fixed inset-x-0 bottom-0 z-[100] overflow-hidden rounded-t-[26px] border-t border-white/20 bg-[#00B4D8] shadow-[0_-8px_24px_rgba(0,0,0,.14)]" aria-label="Navegación móvil del cliente">
        <div class="mx-auto grid max-w-md grid-cols-5"><button v-for="item in items" :key="item.key" type="button" class="flex min-w-0 flex-col items-center gap-1 px-1 py-2 text-white transition" :class="active===item.key?'bg-white/15':'text-white/70'" @click="go(item)">
...
<style scoped>
.customer-nav-desktop{display:none}.customer-nav-mobile{display:block}@media (min-width:1024px){.customer-nav-desktop{display:flex}.customer-nav-mobile{display:none}}
</style>
