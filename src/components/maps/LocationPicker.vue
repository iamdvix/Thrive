<script setup>
// Selector de ubicación exacta. El formulario puede desplazarse sobre el mapa sin quedar atrapado.
...
const mapEl=ref(null),mapReady=ref(false),interactionEnabled=ref(false);
...
async function initMap(){
    ...
    map=L.map(mapEl.value,{zoomControl:true,scrollWheelZoom:false,dragging:false,touchZoom:false,doubleClickZoom:false,boxZoom:false,keyboard:false}).setView(center,hasPoint()?15:8);
...
<div class="min-w-0">
    ...
    <div class="thrive-picker-shell relative w-full overflow-hidden rounded-[22px] border border-[#90E0EF]/50 bg-[#EAF9FC]" :class="{'is-interacting':interactionEnabled}" style="height:340px;min-height:340px">
        <div ref="mapEl" class="thrive-picker-map absolute inset-0 h-full w-full"></div>
        <button v-if="mapReady" type="button" class="thrive-picker-mode" @click.stop="setInteraction(!interactionEnabled)">{{ interactionEnabled?'Listo':'Mover mapa' }}</button>
    ...
    <p class="mt-2 text-xs leading-5 text-gray-400">Toca el mapa para colocar el punto. Para desplazarlo, pulsa “Mover mapa”; cuando termines pulsa “Listo”. La rueda y el gesto vertical vuelven a desplazar la página normalmente.</p>
</div>
</template>

<style>
.thrive-picker-shell{touch-action:pan-y;overscroll-behavior:auto}.thrive-picker-shell.is-interacting{touch-action:none}.thrive-picker-map.leaflet-container{touch-action:pan-y;overscroll-behavior:auto}.thrive-picker-shell.is-interacting .thrive-picker-map.leaflet-container{touch-action:none}.thrive-picker-mode{position:absolute;right:12px;top:12px;z-index:700;border:1px solid rgba(0,180,216,.24);border-radius:9999px;background:rgba(255,255,255,.96);padding:8px 12px;color:#0077b6;font-size:11px;font-weight:900;box-shadow:0 4px 14px rgba(0,0,0,.08)}.thrive-picker-marker{background:transparent!important;border:0!important;touch-action:none!important}.thrive-picker-dot{width:36px;height:36px;border-radius:9999px;background:#00b4d8;border:5px solid #fff;box-shadow:0 6px 18px rgba(0,119,182,.38)}
</style>
